...
{
	if (region.P.B === region.V.B)
	{
		return 'on line ' + region.P.B;
	}
	return 'on lines ' + region.P.B + ' through ' + region.V.B;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aM, request.aZ, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {X: fragment, _: host, ae: path, ag: port_, ak: protocol, al: query};
	});
...
var $author$project$Main$Model = F7(
	function (minRating, minPrice, maxPrice, radius, minUserRatings, location, results) {
		return {K: location, w: maxPrice, x: minPrice, C: minRating, D: minUserRatings, E: radius, F: results};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{aB: false, aD: r.aD, aH: r.aH, Y: r.Y, aM: r.aM, aW: r.aW, av: r.av, aZ: r.aZ}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{aD: $elm$http$Http$emptyBody, aH: r.aH, Y: _List_Nil, aM: 'GET', aW: $elm$core$Maybe$Nothing, av: $elm$core$Maybe$Nothing, aZ: r.aZ});
};
...
var $author$project$Main$Place = F4(
	function (name, rating, price_level, address) {
		return {S: address, ac: name, ah: price_level, am: rating};
	});
...
var $author$project$Main$placeDecoder = A5(
	...
	A2($elm$json$Json$Decode$field, 'adr_address', $elm$json$Json$Decode$string));
var $author$project$Main$mainDecoder = $elm$json$Json$Decode$list(
	$elm$json$Json$Decode$nullable($author$project$Main$placeDecoder));
var $author$project$Main$getResults = function (model) {
	var _v0 = model.K;
	var lat = _v0.a;
	var _long = _v0.b;
	var locationAsString = A2(
		$elm$core$String$join,
		',',
		A2(
			$elm$core$List$map,
	...
	return $elm$http$Http$get(
		{
			aH: A2($elm$http$Http$expectJson, $author$project$Main$ResultsReceived, $author$project$Main$mainDecoder),
			aZ: 'https://europe-west1-findcheaprestaurants.cloudfunctions.net/restaurants?' + params
		});
};
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 2:
				var price = msg.a;
				return (_Utils_cmp(price, model.w) < 1) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{x: price}),
			...
			case 3:
				var price = msg.a;
				return (_Utils_cmp(price, model.x) > -1) ? _Utils_Tuple2(
					_Utils_update(
						model,
						{w: price}),
					$elm$core$Platform$Cmd$none) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
			...
			default:
				...
				if (!result.$) {
					var rs = result.a;
					var noNulls = A2($elm$core$List$filterMap, $elm$core$Basics$identity, rs);
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								F: $author$project$Main$Loaded(noNulls)
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					return _Utils_Tuple2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$String$fromFloat(place.am))
					])),
				A2(
				$elm$html$Html$td,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$String$fromInt(place.ah))
					])),
				A2(
				$elm$html$Html$td,
				_List_Nil,
												...
												[
													...
													_List_fromArray(
														[
															...
															_List_fromArray(
																[
																	$elm$html$Html$text('Price level')
																])),
															$elm$html$Html$text('£')
														])),
													A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$tablePart(model.F))
			]));
};
var $author$project$Main$main = $elm$browser$Browser$element(
	{aL: $author$project$Main$init, aV: $author$project$Main$subscriptions, aY: $author$project$Main$update, a_: $author$project$Main$view});
