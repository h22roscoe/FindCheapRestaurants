...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {ai: fragment, am: host, au: path, aw: port_, aA: protocol, aB: query};
	});
...
var $author$project$Main$filterResultsList = F2(
	function (model, rs) {
		var moreThanMinUserRatings = function (x) {
			return _Utils_cmp(x.aQ, model.A) > -1;
		};
		var moreThanMinRating = function (x) {
			return _Utils_cmp(x.U, model.z) > -1;
		};
		var moreThanMinPrice = function (x) {
			return _Utils_cmp(
				A2($elm$core$Maybe$withDefault, 4, x.P),
				model.v) > -1;
		};
		var lessThanMaxPrice = function (x) {
			return _Utils_cmp(
				A2($elm$core$Maybe$withDefault, 0, x.P),
				model.t) < 1;
		};
		var allConds = $author$project$Main$and(
			_List_fromArray(
				[lessThanMaxPrice, moreThanMinPrice, moreThanMinUserRatings, moreThanMinRating]));
		var wantedResults = A2($elm$core$List$filter, allConds, rs);
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'minRating',
				$elm$json$Json$Encode$float(model.z)),
				_Utils_Tuple2(
				'minPrice',
...
var $author$project$Main$Place = F5(
	function (name, rating, priceLevel, address, userRatingsTotal) {
		return {_: address, as: name, P: priceLevel, U: rating, aQ: userRatingsTotal};
	});
...
var $author$project$Main$placeDecoder = A6(
	...
	A2($elm$json$Json$Decode$field, 'user_ratings_total', $elm$json$Json$Decode$int));
		...
		switch (msg.$) {
			...
			case 1:
				...
				if (_v1.$ === 3) {
					var rs = _v1.a;
					return A2(
						$author$project$Main$filterResultsList,
						_Utils_update(
							model,
							{z: rating}),
						rs);
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$elm$core$String$fromFloat(place.U))
					])),
				A2(
				$elm$html$Html$td,
				_List_Nil,
				_List_fromArray(
					[
						...
						$elm$core$String$fromInt(
							A2($elm$core$Maybe$withDefault, 0, place.P)))
					])),
				A2(
				$elm$html$Html$td,
				_List_Nil,
